use crate::{app::{CurrentScreen, CurrentlyEditing}, ui::*};
...
pub fn render_input_ui(frame: &mut Frame, app: &mut App) {
    if let Some(editing) = &app.get_currently_editing() {
        let title_text = match app.get_current_screen() {
            CurrentScreen::Adding  => "Add a new project",
            CurrentScreen::Editing => "Edit selected project",
            _ => {panic!("This point shouldn't be reached, something went very wrong!")}
        };

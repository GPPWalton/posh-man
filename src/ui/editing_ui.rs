...
pub fn render_editing_ui(frame: &mut Frame, app: &mut App, selected_index: usize) {
    if let Some(editing) = &app.get_currently_editing() {
        ...
        if let Some(edit_mode) = app.get_currently_editing(){
            ...
            CurrentlyEditing::Project(_) => app.get_input_array()[0].to_string(),
            CurrentlyEditing::Size(_) => app.get_input_array()[1].to_string(),
            CurrentlyEditing::Cost(_) => app.get_input_array()[2].to_string(),
            CurrentlyEditing::WholeArmy(_) => app.get_input_array()[3].to_string(),
            CurrentlyEditing::AssemblyRequired(_) => app.get_input_array()[4].to_string(),
            CurrentlyEditing::KitbashRating(_) => app.get_input_array()[5].to_string(),
            CurrentlyEditing::PaintingLevel(_) => app.get_input_array()[6].to_string(),
            CurrentlyEditing::ComplexityRating(_) => app.get_input_array()[7].to_string(),
            CurrentlyEditing::Priority(_) => app.get_input_array()[8].to_string(),
            CurrentlyEditing::Status(_) => app.get_input_array()[9].to_string(),
            CurrentlyEditing::IsOwned(_) => app.get_input_array()[10].to_string(),
           };
